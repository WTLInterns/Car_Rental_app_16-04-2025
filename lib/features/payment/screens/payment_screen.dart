...
import 'package:worldtriplink/features/booking/models/booking_model.dart';
import 'package:worldtriplink/features/payment/models/discount_model';
...
class _PaymentScreenState extends State<PaymentScreen> {
 ...
 final actualCode = _discountData!.discountCoupon?.toUpperCase() ?? '';

 if (enteredCode == actualCode) {
 setState(() {
 _isCouponApplied = true;
 _discountAmount = _discountData!.price ?? 0;
 });
 ...
 try {
 final response = await http.get(Uri.parse('$apiBaseUrl/discount/getAll'));
 ...
 _discountAmount = discount.price ?? 0;
 });
 break;
 }
 }
 ...
 Uri.parse('$apiBaseUrl/by-user/$userId'),
 );
 ...
 if ((effectiveUserId == null || effectiveUserId.isEmpty) && prefs.containsKey('userData')) {
 final userData = prefs.getString('userData');
 if (userData != null) {
 final parsedData = json.decode(userData);
 if (parsedData['id'] != null) {
 ...
 final uri = Uri.parse('$apiBaseUrl/bookingConfirm').replace(queryParameters: requestParams);
 ...
 const SizedBox(height: 8),
 if (_isCouponApplied)
 _buildFareRow('Discount', '-₹$_discountAmount'),
 const Divider(),
 ...
 ScaffoldMessenger.of(context).showSnackBar(
 const SnackBar(content: Text("You are Already Used")),
 );
 }
 },
 child: const Text(
 ...
 children: [
 Text(
 _discountData?.discountCoupon?.toUpperCase() ?? '',
 style: const TextStyle(
 fontSize: 18,
 letterSpacing: 2,
 fontWeight: FontWeight.bold,
 ...
 GestureDetector(
 onTap: () {
 final coupon = _discountData?.discountCoupon?.toUpperCase() ?? '';
