...
class _ProfileScreenState extends State<ProfileScreen>
    ...
    if (_userId != null) {
      try {
        ...
        if (response.statusCode == 200) {
          ...
          setState(() {
            _userName = _userProfile?.username ?? _userName;
            _userEmail = _userProfile?.email ?? _userEmail;
            _userPhone = _userProfile?.phone ?? _userPhone;
  ...
  Future<void> _updateProfile(User updatedUser) async {
    ...
    // Basic validation
    if (updatedUser.username?.trim().isEmpty ?? true) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Name cannot be empty')),
      );
      return;
    }
    if (updatedUser.email?.trim().isEmpty ??
        true ||
            !RegExp(r'^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$')
                .hasMatch(updatedUser.email ?? '')) {
      ScaffoldMessenger.of(context).showSnackBar(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final prefs = await SharedPreferences.getInstance();
        await prefs.setString('username', _userProfile?.username ?? 'User');
        await prefs.setString(
            'email', _userProfile?.email ?? 'user@example.com');
        await prefs.setString('phone', _userProfile?.phone ?? '+91 0000000000');
                ...
                child: Column(
                  children: [
                    ...
                    _buildSection(
                      ...
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => AboutWTLScreen(),
                          ),
                        );
                      },
                    ),
                    _buildSection(
                      ...
                      icon: MaterialCommunityIcons.account_voice,
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
    ...
    return Expanded(
      child: Column(
        children: [
          ...
          Text(label, style: const TextStyle(color: lightTextColor, fontSize: 12)),
        ],
      ),
    );
  }
